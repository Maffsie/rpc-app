...
class Tailzone(Micro):
    async def get_devices(self):
        network = Tailscale(
            api_key=self.conf.get("vnd_tailscale_apikey"),
            tailnet=self.conf.get("vnd_tailscale_tailnet"),
        )
    ...
    def main(self, *args, **kwargs):
        results = {
            "mode": "periodic"
            if not self.conf.get("tailzone_destroy_all_managed_records", False)
            else "destroy",
            "poll_stats": {
                "count_dns_records": 0,
        ...
        domain = DOdomain(
            token=self.conf.get("vnd_digitalocean_apikey"),
            name=self.conf.get("vnd_digitalocean_domain"),
        )
        ...
        managed = [
            ...
            if record.name == self.conf.get("vnd_digitalocean_managed_record")
            and record.type == "TXT"
        ]
        ...
        pending = [
            host
            for host in devices
            if f"tag:{self.conf.get('vnd_tailscale_tag')}" in host["tags"]
        ...
        removable = [
            host["host"]
            for host in devices
            if f"tag:{self.conf.get('vnd_tailscale_tag')}" not in host["tags"]
        ...
        if self.conf.get("tailzone_destroy_all_managed_records", False):
            removable = [
        ...
        changeset = {
            "destroy": [
                ...
                or (
                    record.name == self.conf.get("vnd_digitalocean_managed_record")
                    and record.type == "TXT"
